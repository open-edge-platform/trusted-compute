...
LANGFUSE_REPOSITORY="${LANGFUSE_REPOSITORY:-https://github.com/langfuse/langfuse.git}"
...
PLUGIN_DIR="${PLUGIN_DIR:-${HOME}/openclaw-x-langfuse-plugin}"
...
langfuse_compose() {
    docker compose --project-directory "${LANGFUSE_DIR}" \
        ...
        --file "${LANGFUSE_DIR}/docker-compose.yml" "$@"
}
...
write_langfuse_config() {
    ...
    if docker compose --project-directory "${LANGFUSE_DIR}" \
        --file "${LANGFUSE_DIR}/docker-compose.yml" ps -q | grep -q .; then
        adopt_existing_config
        return
    fi
...
clone_if_missing "${LANGFUSE_REPOSITORY}" "${LANGFUSE_DIR}"
write_langfuse_config

log "Deploying Langfuse"
langfuse_compose up -d --wait --wait-timeout "${LANGFUSE_WAIT_TIMEOUT:-180}"

clone_if_missing "${PLUGIN_REPOSITORY}" "${PLUGIN_DIR}"
