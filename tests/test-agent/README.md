...
different location. The Compose file defaults to the immutable commit specified
by `LANGFUSE_COMPOSE_REF`; set it to another full commit, or set
`LANGFUSE_COMPOSE_URL` to use another source. If a running Langfuse stack has no
environment file, the script preserves its credentials from the container
