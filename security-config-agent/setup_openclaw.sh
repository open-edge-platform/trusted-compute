...
Options:
  ...
  --dashboard-url         Only issue a new single-use dashboard link for the running deployment
...
# Shares the gateway's network namespace, so only valid once the gateway container exists.
cli() {
    compose run -T --rm openclaw-cli "$@"
}
...
pull_model() {
    local model_dir="$MODELS_DIR/$OVMS_MODEL"

    # graph.pbtxt is written last by the pull, so its presence marks a complete download.
    if [[ -f "$model_dir/graph.pbtxt" ]]; then
        echo "Model '$OVMS_MODEL' already present in $MODELS_DIR; skipping pull."
        echo "  (delete $model_dir to force a re-pull)"
        return
    fi

    echo "Pulling model '$OVMS_MODEL' into $MODELS_DIR (one-off, egress network only)..."
    if ! compose run --rm --no-deps ovms-pull; then
        echo "ERROR: model pull failed." >&2
        exit 1
    fi
    if [[ ! -f "$model_dir/graph.pbtxt" ]]; then
        echo "ERROR: pull finished but $model_dir/graph.pbtxt is missing." >&2
        exit 1
    fi
}
...
wait_for_ovms() {
    local deadline=$((SECONDS + OVMS_READY_TIMEOUT)) ip="" response
    echo "Waiting for OVMS model '${OVMS_MODEL}'..."
    while (( SECONDS < deadline )); do
        [[ -n "$ip" ]] || ip=$(container_ip ovms 2>/dev/null || true)
        if [[ -n "$ip" ]] &&
            response=$(curl -fsS --noproxy '*' --max-time 5 "http://${ip}:8000/v3/models" 2>/dev/null) &&
...
approve_security_node() {
    ...
    while (( SECONDS < deadline )); do
        devices=$(cli devices list --json 2>/dev/null || true)
        ...
        if [[ -n "$request_id" ]]; then
            ...
            cli devices approve "$request_id"
...
approve_security_node_commands() {
    ...
    while (( SECONDS < deadline )); do
        nodes=$(cli nodes status --json 2>/dev/null || true)
        if jq -e --arg name "$SECURITY_NODE_NAME" \
...
wait_for_security_node() {
    ...
    while (( SECONDS < deadline )); do
        nodes=$(cli nodes status --connected --json 2>/dev/null || true)
        if jq -e --arg name "$SECURITY_NODE_NAME" \
...
verify_skills_available() {
    ...
    if ! report=$(cli skills list --agent "$AGENT_ID" --eligible --json); then
    ...
    if ! jq -e 'length == 0' >/dev/null <<<"$missing"; then
        ...
        jq -r '.[] | "  - \(.)"' <<<"$missing" >&2
        echo >&2
        cli skills list --agent "$AGENT_ID" --verbose || true
        exit 1
    fi
...
configure_node_approvals() {
    ...
    cli approvals set --node "$SECURITY_NODE_NAME" --stdin <<'EOF'
{
  "version": 1,
  "defaults": {
    "security": "allowlist",
...
container_ip() {
    # Every service sits on exactly one network at runtime, so the range yields a single address.
    docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' \
        "$(compose ps -q "$1")"
}

print_dashboard_url() {
    local ip handoff url

    ip="$(container_ip openclaw-gateway)"
    if [[ ! "$ip" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        echo "WARNING: could not determine the gateway container IP." >&2
        return
    fi

    # Published ports are unreachable on the internal network, so the UI is reached via the
    # container IP; the one-time bootstrap link pre-approves the browser that opens it.
    if ! handoff=$(cli dashboard --json 2>/dev/null) ||
        ! url=$(jq -er 'select(.ok == true) | .browserUrl' <<<"$handoff"); then
        echo "WARNING: could not issue a dashboard handoff link." >&2
        echo "  UI:     http://${ip}:${GATEWAY_PORT} (paste the token, then approve with: devices approve)"
        return
    fi

    echo "  UI:     ${url/127.0.0.1/$ip}"
    echo "          (single-use, expires in ~10 min; pre-approves the browser that opens it)"
    echo "  Re-issue: $0 --dashboard-url"
}

print_ovms_url() {
    local ip
    ip=$(container_ip ovms 2>/dev/null || true)
    [[ -n "$ip" ]] && echo "  OVMS:   http://${ip}:8000/v3/models"
}

reissue_dashboard_url() {
    if [[ ! -f "$ENV_FILE" ]]; then
        echo "ERROR: $ENV_FILE not found; run the full deployment first." >&2
        exit 1
    fi
    if [[ -z "$(compose ps -q --status running openclaw-gateway)" ]]; then
        echo "ERROR: the openclaw-gateway container is not running." >&2
        exit 1
    fi
    print_dashboard_url
}
...
print_summary() {
    ...
    echo "Gateway port:   ${GATEWAY_PORT} (internal network, reached via container IP)"
...
main() {
    parse_args "$@"
    if [[ $DASHBOARD_URL_ONLY -eq 1 ]]; then
        reissue_dashboard_url
        return
    fi
    load_model_config
